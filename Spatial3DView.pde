...
public class Spatial3DView extends View {
  ...
  float rotation = 0;
  float zoom = 1.0;
  PVector middle = new PVector(0.5,0.5);
  ...
  public void drawView() {
    ...
    scene.stroke(255,255,0);
    for (int i = 0; i < model.getMap().path.size()-1; i++){
      PVector point = model.getMap().path.get(i);
      PVector nextPoint = model.getMap().path.get(i+1);
      //middle.x = nextPoint.x;
      //middle.y = nextPoint.y;
      //if (model.getMap().path.size() % 2 ==1){
      //middle = model.getMap().path.get((i));
      //}
      middle = nextPoint;
      
      
      drawLineOnMap(scene, point.x, point.y, nextPoint.x, nextPoint.y);
    ...
    scene.noStroke();
    scene.fill(0,255,0);
    //drawSphereOnMap(scene, middle.x, middle.y, 10);

    drawSphereOnMap(scene, middle.x, middle.y, 10);
    //scene.fill(0,255,0);
    //drawBoxOnMap(scene, middle.x, middle.y, 10);
